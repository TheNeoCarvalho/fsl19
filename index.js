...
app.get("/admin/vagas/update/:id", async (req, res) => {
  ...
  const vaga = await db.get(
    `SELECT * FROM vaga WHERE id = ${id};`
  );
  let cat = vaga.categoria;
  const categoria = await db.get(
    `SELECT * FROM categoria WHERE id = ${cat};`
  );
...
app.post("/admin/vagas/update/:id", async (req, res) => {
  ...
  const db = await dbConnection;
  await db.run(
    `UPDATE vaga SET titulo= '${titulo}', descricao = '${descricao}', categoria = '${categoria}' WHERE id = ${idUrl};`
  );
  res.redirect("/admin/vagas");
})
...
app.post("/admin/categorias/update/:id", async (req, res) => {
  ...
  const db = await dbConnection;
  await db.run(
    `UPDATE categoria SET categoria = '${categoria}' WHERE id = ${idUrl};`
  );
  res.redirect("/admin/categorias")
})
